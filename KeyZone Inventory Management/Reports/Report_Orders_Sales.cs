...
{
    public partial class Report_Orders_Sales : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public Report_Orders_Sales()
        {
            InitializeComponent();
        }

        private void Report_Orders_Sales_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "SP_Orders_Invoice_Report";
                cmd.Parameters.AddWithValue("@Order_Number", FRM_Report_Orders_Sales.fRM_Report.textBox_Orders_No.Text);
                cmd.Parameters.AddWithValue("@Myear", FRM_Report_Orders_Sales.fRM_Report.textBox_Year.Text);

                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(m, m.Tables[0].TableName);
                ReportDataSource reportDataSource = new ReportDataSource("DS_Orders_Sales_Report", m.Tables[0]);
