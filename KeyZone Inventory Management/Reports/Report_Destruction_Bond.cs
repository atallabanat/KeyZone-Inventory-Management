...
{
    public partial class Report_Destruction_Bond : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public Report_Destruction_Bond()
        {
            InitializeComponent();
        }

        private void Report_Destruction_Bond_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "SP_Destruction_Bond_Report";
                cmd.Parameters.AddWithValue("@Bond_No", FRM_Report_Destruction_Bond.fRM_Report.textBox_Bond_No.Text);
                cmd.Parameters.AddWithValue("@Myear", FRM_Report_Destruction_Bond.fRM_Report.textBox_Year.Text);

                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(m, m.Tables[0].TableName);

                this.reportViewer1.LocalReport.SetParameters(reportParameters);
                ReportDataSource reportDataSource = new ReportDataSource("DS_Destruction_Bond_Report", m.Tables[0]);
