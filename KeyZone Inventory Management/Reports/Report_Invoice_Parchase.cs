...
{
    public partial class Report_Invoice_Parchase : Form
    {

        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public Report_Invoice_Parchase()
        {
            InitializeComponent();
        }
        ...
        private void testtt_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "SP_Invoice_Parchase_Report";
                cmd.Parameters.AddWithValue("@Invoice_No", FRM_Report_Invoice_Parchase.fRM_Report.textBox_Invoice__Number.Text);
                cmd.Parameters.AddWithValue("@Myear", FRM_Report_Invoice_Parchase.fRM_Report.textBox_Year.Text);

                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(m, m.Tables[0].TableName);

                this.reportViewer1.LocalReport.SetParameters(reportParameters);
                ReportDataSource reportDataSource = new ReportDataSource("DS_Invoice_Parchase_Report", m.Tables[0]);
