...
{
    public partial class FRM_Report_Invoice_Sales : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public static FRM_Report_Invoice_Sales fRM_Report;
        //public virable
        int Invoice_No;
        public FRM_Report_Invoice_Sales()
        {
            fRM_Report = this;
            InitializeComponent();
        }

        private void FRM_Report_Invoice_Sales_Load(object sender, EventArgs e)
        {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                SqlCommand cmd21 = new SqlCommand("select count( *) as cc from Invoice_Sales where Invoice_No=@Invoice_No and Myear=@Myear", con);
                ...
                Report_Invoice_Sales Report_Invoice_Sales2 = new Report_Invoice_Sales();
                Report_Invoice_Sales2.Show();
            }
            catch (Exception ee)
            {
                con.Close();
                MessageBox.Show("يرجى تصوير الخطأ ومراجعة مدير النظام ، شكرا" + ee.Message, "ERROR 1001 FRM_Report_Invoice_Sales", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btn_View_Bond_No_Click(object sender, EventArgs e)
        {
            if (textBox_Year.Text != "")
            {
                Grid_Invoice_Sales.SCR_FRM_Report_Invoice_Sales = true;
                Grid_Invoice_Sales Grid_Invoice_Sales2 = new Grid_Invoice_Sales();
                Grid_Invoice_Sales2.ShowDialog();
                Grid_Invoice_Sales.SCR_FRM_Report_Invoice_Sales = false;
            }
        }
    }
}
