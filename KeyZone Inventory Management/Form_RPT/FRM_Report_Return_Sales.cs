...
{
    public partial class FRM_Report_Return_Sales : Form
    {

        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public static FRM_Report_Return_Sales fRM_Report;
        //public virable
        int bond_no;

        public FRM_Report_Return_Sales()
        {
            fRM_Report = this;
            InitializeComponent();
        }

        private void FRM_Report_Return_Sales_Load(object sender, EventArgs e)
        {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                SqlCommand cmd21 = new SqlCommand("select count( *) as cc from Return_Sales where Bond_No=@Bond_No and Myear=@Myear", con);
                ...
                Report_return_Sales Report_return_Sales2 = new Report_return_Sales();
                Report_return_Sales2.Show();

            }
            catch (Exception ee)
            {
                con.Close();
                MessageBox.Show("يرجى تصوير الخطأ ومراجعة مدير النظام ، شكرا" + ee.Message, "ERROR 1001 FRM_Report_Return_Sales", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btn_View_Bond_No_Click(object sender, EventArgs e)
        {
            if (textBox_Year.Text != "")
            {
                Grid_Return_Bound_Sales.SCR_FRM_Return_Sales = true;
                Grid_Return_Bound_Sales Grid_Return_Bound_Sales2 = new Grid_Return_Bound_Sales();
                Grid_Return_Bound_Sales2.ShowDialog();
                Grid_Return_Bound_Sales.SCR_FRM_Return_Sales = false;
            }
        }
    }
}
