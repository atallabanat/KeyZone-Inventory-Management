...
{
    public partial class FRM_Report_Out_Bond : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public static FRM_Report_Out_Bond fRM_Report;
        //public virable
        int bond_no;
        public FRM_Report_Out_Bond()
        {
            fRM_Report = this;
            InitializeComponent();
        }

        private void FRM_Report_Out_Bond_Load(object sender, EventArgs e)
        {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                SqlCommand cmd21 = new SqlCommand("select count( *) as cc from Out_Bond where Bond_No=@Bond_No and Myear=@Myear", con);
                ...
                Report_Out_Bond Report_Out_Bond2 = new Report_Out_Bond();
                Report_Out_Bond2.Show();
            }
            catch (Exception ee)
            {
                con.Close();
                MessageBox.Show("يرجى تصوير الخطأ ومراجعة مدير النظام ، شكرا" + ee.Message, "ERROR 1001 FRM_Report_Out_Bond", MessageBoxButtons.OK, MessageBoxIcon.Error);
