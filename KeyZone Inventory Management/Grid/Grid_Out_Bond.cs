...
namespace KeyZone_Inventory_Management.Grid
{
    public partial class Grid_Out_Bond : Form
    {
        ...
        //public virable
        public static Boolean SCR_FRM_Report_Out_Bond;
        ...
        private void Grid_Out_Bond_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (SCR_FRM_Report_Out_Bond == true)
                {
                    using (SqlCommand Cmd = con.CreateCommand())
                    {
                        Cmd.CommandType = CommandType.StoredProcedure;
                        Cmd.CommandText = "SP_SEARCH_DVG_Out_Bond_GRID";
                        Cmd.Parameters.Add(new SqlParameter("@Myear", FRM_Report_Out_Bond.fRM_Report.textBox_Year.Text));
                ...
                else
                {
                    using (SqlCommand Cmd = con.CreateCommand())
                    {
                        Cmd.CommandType = CommandType.StoredProcedure;
                        Cmd.CommandText = "SP_SEARCH_DVG_Out_Bond_GRID";
                        Cmd.Parameters.Add(new SqlParameter("@Myear", Out_Bond.out_B.textBox_Year.Text));
                        SqlDataAdapter da = new SqlDataAdapter(Cmd);
                        da.Fill(dataTable);
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.Rows.Count > 0)
                {
                    ...
                    if (SCR_FRM_Report_Out_Bond == true)
                    {
                        FRM_Report_Out_Bond.fRM_Report.textBox_Bond_No.Text = bond_no;
                        SCR_FRM_Report_Out_Bond = false;
                    }
                    else
                    {
                        Out_Bond.out_B.textBox_Bond_No.Text = bond_no;
                        Out_Bond.out_B.addScren();
                    }
                }
